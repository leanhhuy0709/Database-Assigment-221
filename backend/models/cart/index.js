...
Cart.TotalPrice = (data, result) => {
  db.query(`select cal_total_price(${data.Cart_id})`, (error, totalPrice) => {
    if (error) {
      ...
      //console.log(totalPrice[0][`cal_total_price(${data.Cart_id})`]);
      result(totalPrice[0][`cal_total_price(${data.Cart_id})`]);
    }
  });
};
...
Cart.getCombo = (data, result) => {
  db.query(`call getCartCombo(${data.Cart_id})`, (error, combo) => {
    if (error) {
      //console.log(error);
      result(error.sqlMessage);
    } else {
