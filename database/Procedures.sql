...
drop procedure if exists getCartFood;

delimiter \\
create procedure getCartFood(IN Cart_id int)
begin
    select * from `Food_Cart`
    join `Food` on `Food`.F_id = `Food_Cart`.F_id
    where `Food_Cart`.Cart_id = Cart_id;
end \\
delimiter ;

-- call getCartFood(Cart_id);

drop procedure if exists getCartCombo;

delimiter \\
create procedure getCartCombo(IN Cart_id int)
begin
    select * from `Combo_Cart`
    join `Combo` on `Combo`.Cb_id = `Combo_Cart`.Cb_id
    where `Combo_Cart`.Cart_id = Cart_id;
end \\
delimiter ;
